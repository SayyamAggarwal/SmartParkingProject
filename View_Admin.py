...
class Main:
    def __init__(self):
        ...
        self.mainLabel = Label(self.root, text="View Admin", font=('Arial', 26, 'bold'))
    ...
    def getValues(self):
        q = "select id, name, email, mobile, role from admin_table"
        self.cr.execute(q)
    ...
    def searchAdmin(self):
        text = self.txt0.get()
        q0 = f"select id, name, email, mobile, role from admin_table where name like '%{text}%' or email like '%{text}%' or mobile like '%{text}%'"
        self.cr.execute(q0)
        result = self.cr.fetchall()
        for item in self.adminTable.get_children():
            self.adminTable.delete(item)
    ...
    def openUpdateWindow(self, e):
        ...
        self.lb4 = Label(self.formFrame, text="Admin ID", font=self.font)
        ...
        self.txt4.insert(0, data[0])
        self.txt4.config(state='readonly')

        self.lb5 = Label(self.formFrame, text="Select Role", font=self.font)
    ...
    def updateAdmin(self):
        ...
        if name == "" or email == "" or mobile == "" or role == "":
            ...
            if verifyMobile(mobile) == True and verifyEmail(email) == True:
                q2 = f"update admin_table set name='{name}', email='{email}', mobile='{mobile}', role='{role}' where id={id}"
                self.cr.execute(q2)
                self.conn.commit()
                self.root1.destroy()
    ...
    def deletAdmin(self):
        id = self.txt4.get()
        q4 = f"delete from admin_table where id='{id}'"
        self.cr.execute(q4)
        self.conn.commit()
        self.root1.destroy()
