...
class Main:
    def __init__(self):
        ...
        self.mainLabel = Label(self.root, text="View Transaction", font=('Arial', 26, 'bold'))
    ...
    def getValues(self):
        q = "select id,vehicle_number, vehicle_id, date, time, admin_id, amount from transaction"
        self.cr.execute(q)
    ...
    def searchAdmin(self):
        text = self.txt0.get()
        q0 = f"select id,vehicle_number, vehicle_id, date, time, admin_id, amount from transaction where vehicle_id like '%{text}%' or admin_id like '%{text}%' or date like '%{text}%'"
        self.cr.execute(q0)
        result = self.cr.fetchall()
        for item in self.adminTable.get_children():
            self.adminTable.delete(item)
    ...
    def updateAdmin(self):
        ...
        vehicle_number =amount = self.txt7.get()
        ...
        if vehicle_id == "" or date == "" or time == "" or admin_id == "" or amount =="" :
            ...
            q2 = f"update transaction set vehicle_number='{vehicle_number}', vehicle_id='{vehicle_id}', date='{date}',time='{time}', admin_id='{admin_id}',amount='{amount}' where id={id}"
            self.cr.execute(q2)
            self.conn.commit()
            self.root1.destroy()
    ...
    def deletAdmin(self):
        id = self.txt4.get()
        q4 = f"delete from transaction where id='{id}'"
        self.cr.execute(q4)
        self.conn.commit()
        self.root1.destroy()
