...
class Add_ParkingPlaza:
    def __init__(self):
        ...
        self.root.title('Add PARKING PLAZA')
        ...
        self.mainLabel = tkinter.Label(self.root,text='PARKING PLAZA',fg=self.framebackground,font=('Helvetica 12 bold',16,'bold'))
        ...
        self.lb3 = tkinter.Label(self.form, text='Enter Password', font=('Helvetica 12 bold', 12, 'bold'))
        ...
        self.lb4 = tkinter.Label(self.form, text='Enter Mobile', font=('Helvetica 12 bold', 12, 'bold'))
        ...
        self.lb5 = tkinter.Label(self.form, text='Enter Location', font=('Helvetica 12 bold', 12, 'bold'))
        ...
        self.lb6 = tkinter.Label(self.form, text='STATUS', font=('Helvetica 12 bold', 12, 'bold'))
        ...
        self.cb1=tkinter.ttk.Combobox(self.form,state='readonly',values=["Parked"],width=40,height=2,font=('Helvetica 12 bold', 12, 'bold'))
        self.cb1.grid(row=5,column=1,padx=10,pady=10)
    ...
    def AddParkingPlaza(self):
            ...
            if verifyEmail(email) == True and verifyMobile(mobile) == True:
                q1 = f"select*from parking_plaza where email='{email}'or mobile='{mobile}'"
                self.cr.execute(q1)
                result = self.cr.fetchall()
                print(result)
                if len(result) == 0:

                    q = f"insert into parking_plaza values (null,'{name}','{email}','{password}','{mobile}','{location}','{status}')"
                    self.cr.execute(q)
                    self.conn.commit()
