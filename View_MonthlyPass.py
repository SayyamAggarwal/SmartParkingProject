...
class Main:
    def __init__(self):
        ...
        self.mainLabel = Label(self.root, text="View MONTHLY PASS", font=('Arial', 26, 'bold'))
    ...
    def getValues(self):
        q = "select id,category_name, price, description from monthly_pass"
        self.cr.execute(q)
    ...
    def searchAdmin(self):
        text = self.txt0.get()
        q0 = f"select id, category_name, price, description from monthly_pass where category_name like '%{text}%' or description like '%{text}%' or price like '%{text}%'"
        self.cr.execute(q0)
        result = self.cr.fetchall()
        for item in self.adminTable.get_children():
            self.adminTable.delete(item)
    ...
    def updateAdmin(self):
        ...
        if category_name == "" or price == "" or description == "":
            ...
            q2 = f"update monthly_pass set category_name='{category_name}', price='{price}',description='{description}' where id={id}"
            self.cr.execute(q2)
            self.conn.commit()
            self.root1.destroy()
